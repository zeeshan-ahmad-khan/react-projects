...
function Message({ message }) {
    ...
    return (
        <div className={`flex items-end space-x-2 relative ${isUserMessage && "justify-end"}`}>
            ...
            ${isUserMessage ? 'rounded-br-none bg-blue-300' : 'rounded-bl-none bg-pink-400'}`}>
            ...
            <p className={`absolute -bottom-5 text-xs ${isUserMessage ? "text-blue-300" : "text-pink-400"}`}>{message.get("username")}</p>
        </div>
    )
}
